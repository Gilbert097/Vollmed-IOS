...
protocol HttpClient {
    func sendRequest<T: Decodable>(endpoint: Enpoint, responseModel: T.Type?) async -> Result<T?, RequestError>
}
...
extension HttpClient {
    func sendRequest<T: Decodable>(endpoint: Enpoint, responseModel: T.Type?) async -> Result<T?, RequestError> {
        ...
        do {
            ...
            case 200...299:
                guard let responseModel = responseModel else { return .success(nil)}
